...
Solidify.prototype.compile = function (rawTemplate) {
    ...
    var regex = /\{\{ *#?solidify-(\w+)[^\}]*\}\}/, s = rawTemplate;
    while (s.match(regex)) {
        s = s.replace(regex, function (__, name) {
    ...
    while (template.match(regex))
        template = template.replace(regex, function (expr) { return expr.split(' ').join(''); });
...
var compileUrl = function (url, context) {
    ...
    while (pathname.match(regex)) {
        pathname = pathname.replace(regex, function (__, param) {
    ...
    while (query.match(regex)) {
        query = query.replace(regex, function (__, param) {
            if (param && param.length > 1 && context[param.substring(1)])
